...
def update_torrent(club, comp, torrent, date, time, status):
    conn = pymysql.connect(host=host, port=port, user=user, passwd=passwd, db=db)
    cur = conn.cursor()
    cur.execute("UPDATE utorrent SET time='{0}', status='{1}', date = '{5}' WHERE club={2} "
                "and comp = {3} and torrent = '{4}'".format(time, status, club, comp, torrent, date))
    cur.close()
    conn.close()
...
def select_torrent(club, sdate, fdate):
    ...
    conn = pymysql.connect(host=host, port=port, user=user, passwd=passwd, db=db)
    cur = conn.cursor()
    cur.execute("SELECT comp,date,time,torrent,status FROM utorrent where club={0} and date >= '{1}' "
                "and date <= '{2}' order by torrent,comp".format(club, sdate, fdate))
    d = cur.fetchall()
    cur.close()
    conn.close()
    return d
